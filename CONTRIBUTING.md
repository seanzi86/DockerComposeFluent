...
- Add or extend a golden-file scenario when changing what YAML gets emitted: add a static method marked `[Scenario]` to the `<Feature>Scenarios` class in `test/DockerComposeFluent.Tests.Unit/Golden/Scenarios` (create the class for a new feature; scenarios are discovered by reflection, so there is no list to update). The class name gives the feature folder and the method name the fixture, so `PortsScenarios.Forms` is `Golden/Fixtures/ports/forms.verified.yml`. Then run `dotnet test`. Golden files are managed by [Verify](https://github.com/VerifyTests/Verify): a new or changed scenario fails and writes a `<name>.received.yml` next to its `<name>.verified.yml` fixture in `Golden/Fixtures/<feature>`. Review the difference, then accept it by replacing the `.verified.yml` file with the `.received.yml` one (or run `dotnet tool install -g verify.tool` once, then `dotnet verify accept -y`), and review the diff before committing. `.received.yml` files are git-ignored.
- The README quick start is the `readme/quick-start` golden scenario: its code sits between the `README:begin` and `README:end` markers in `Golden/Scenarios/ReadmeScenarios.cs`, and its YAML is `Golden/Fixtures/readme/quick-start.verified.yml`. A test fails if `README.md` stops matching either, so change all three together. Update the short "What is supported" list in the README when a property is added.
